...
function processProjectToDB() {
		...
		then
				...
				cp "${PRM_MULTIQCPROJECT_DIR}/multiqc_picard_HsMetrics.txt" "${CHRONQC_TMP}/${_project}.multiqc_picard_HsMetrics.txt"
				...
				perl -pe 's|SAMPLE|SAMPLE_NAME2|' "${CHRONQC_TMP}/${_project}.multiqc_picard_HsMetrics.txt" > "${CHRONQC_TMP}/${_project}.2.multiqc_picard_HsMetrics.txt"
				...
				then
						...
						log4Bash 'INFO' "${LINENO}" "${FUNCNAME:-main}" '0' "Importing ${_project}.multiqc_picard_HsMetrics.txt"
						chronqc database --update --db "${CHRONQC_DATABASE_NAME}/chronqc_db/chronqc.stats.sqlite" \
						"${CHRONQC_TMP}/${_project}.2.multiqc_picard_HsMetrics.txt" \
						--db-table HsMetrics \
